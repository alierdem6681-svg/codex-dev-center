...
Production readiness suite uzun dry-run JSON stdout'unu kesmeden saklayacak sekilde genisletildi ve prefixed JSON payload parsing regresyon testi eklendi. Superseded/cancelled/final-reconciled duplicate parent task'larindan backlog continuation uretilmesi `cto_autonomous_delivery.backlog_candidate_reason()` guard'i ile engellendi. Direct CTO action mode "basla/uygula/gelistirme yap" sinyalli isleri plan-only backlog yerine `repo_apply_allowed` implementation task olarak acar ve CLI `--help` yanlislikla gorev acmaz. Davranis `tests/test_runtime_status_model.py` icindeki regresyon testleriyle sabitlendi; state_templates module registry/settings/action catalog kayitlari guncellendi. Production deploy, staging deploy, secret/env/token/private key, IAM, billing, DNS/firewall, destructive database veya Google Ads live mutate islemi bu commit icinde yapilmadi.

## 2026-06-04 Worker Dispatch v2 Stale Claim Retry Apply

Worker Dispatch v2'nin kalan retry/re-dispatch parcasi tamamlandi. `supervisor_cli dispatch` artik stale `ASSIGNED/RUNNING` claim kayitlarini idle worker atamasindan once denetler. Worker state ayni task icin aktif `ASSIGNED/RUNNING/REVIEWING` sahiplik gosteriyorsa redispatch yapmaz.

Aktif sahiplik yoksa ve `attempt < max_attempts` ise ayni task `attempt + 1` ile yeniden dispatch edilir; limit dolduysa `FAILED_TIMEOUT` terminal sonucuna iner ve `last_error_code=stale_claim_timeout` yazilir. Davranis `tests/test_runtime_status_model.py` icindeki stale retry, max-attempt terminal ve active-owner guard regresyon testleriyle sabitlendi.

Test sonucu: compile PASS, WorkerStatusModelTest PASS, `tests.test_runtime_status_model` PASS, full `unittest discover -s tests` PASS, dry-run production readiness PASS. Production deploy, staging deploy, runtime state/log/report mutasyonu, secret/env/token/private key, IAM, billing, DNS/firewall, destructive database veya Google Ads live mutate yapilmadi.
