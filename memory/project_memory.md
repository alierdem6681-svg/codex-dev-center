...
## 2026-06-04 Quality Gate Retry Safety Visibility Apply

Quality gate retry simülasyonu artık dry-run ve non-mutating kanıtını `safety_status`, `safety_reasons` ve `required_false_flags` alanlarıyla standard raporda görünür kılar. Bu alanlar diagnostiktir; retry simülasyonu non-blocking kalır ve standard kalite kapısı kararını değiştirmez.

Davranış `tests/test_runtime_status_model.py` içinde safety pass ve non-blocking safety fail senaryolarıyla sabitlendi. Production deploy, staging deploy, runtime state/log mutasyonu, secret/env/token/private key, IAM, billing, DNS/firewall, destructive database veya Google Ads live mutate işlemi yapılmadı.
