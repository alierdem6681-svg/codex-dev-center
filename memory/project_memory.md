...
## 2026-06-04 Controlled Apply Stage Plan Contract Apply

Controlled apply pipeline icin proposal seviyesinden repo apply'a gecis asamalari `docs/CONTROLLED_APPLY_PIPELINE.md` icinde kayda alindi. Repo apply raporu artik `Controlled Apply Stage Plan` bolumunde proposal review, patch plan, diff review, secret scan, local tests, report, rollback note ve production deploy durumlarini sirali yazar.

`supervisor/worker_runner.py` bu stage planini `Controlled Apply Checklist` ile `Rollback Note` arasina ekler. Unsafe path durumunda patch plan ve diff review `FAIL`, local tests `NOT_RUN` kalir; davranis `tests/test_runtime_status_model.py` ile sabitlendi.

Bu paket production deploy, staging deploy, runtime state/log/report mutasyonu, secret/env/token/private key, IAM, billing, DNS/firewall, destructive database veya Google Ads live mutate islemi yapmadi. Runtime `state/` dosyalari bu izole clone icinde bulunmadigi icin repo `state_templates/` kayitlari guncellendi.
