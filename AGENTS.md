...
Simulasyon dry-run PASS sayilabilmesi icin `production_deploy_performed=false`, `staging_deploy_performed=false` ve `mutating_cloud_operations_performed=false` alanlari artefact icinde acikca bulunmalidir; eksik veya non-false deger `fail` kabul edilir.
