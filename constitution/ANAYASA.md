...
Kalite kapısı standart raporu mevcut readiness artefact'lerinden `pass` veya `fail` kararı üretir. Eksik artefact, başarısız test veya dry-run dışı simülasyon kanıtı production deploy izni sayılmaz ve canlı mutasyon yetkisi vermez. Geri alma simülasyonu insan raporu dry-run kontratı FAIL ise `FAIL` kalmalıdır.
