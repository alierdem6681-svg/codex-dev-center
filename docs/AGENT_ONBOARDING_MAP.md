...
- `web_panel/static/index.html` Gorevler listesi render oncesinde deterministik siralama uygular; `RUNNING`/`Calisiyor` gorevleri ustte kalir, `DEPLOYED`/canli ve kapali/gecmis gorevler varsayilan listeden gizlenir, `Canliya alinanlari goster` ve `Gecmis gorevleri goster` checkbox'lari ile geri alinabilir sekilde dahil edilir.
