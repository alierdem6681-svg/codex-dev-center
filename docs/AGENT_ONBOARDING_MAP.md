...
- Ana dashboard `Pipeline Stage Akışı` bolumunde `/api/pipeline-flow` payload'unu okur; UI sadece stage state, task sayisi, status ozeti, guvenli task id/worker satiri ve marker ozetlerini gostermelidir.
