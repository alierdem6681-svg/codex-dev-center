...
- Readiness suite dry-run çıktısında `dry_run=true`, `staging_deploy_performed=false` ve `mutating_cloud_operations_performed=false` kanıtı görmeden ön canlı kapısını PASS saymamalı.
