...
- Go kararı yalnızca zorunlu readiness gate'leri, staging health/smoke ve rollback simulation PASS ise verilir.
- No-Go kararı başarısız test/build/security gate, belirsiz rollback hedefi veya kabul edilmemiş kritik riskte verilir.
- Rollback hedef artifact ve config versiyonu release kaydında doğrulanmış olmalıdır.
- Rollback sonrası post-rollback health check sonucu kapanış notuna yazılmalıdır.
- Final production adımı GitHub Actions manuel workflow kapısından ayrılmamalıdır.
