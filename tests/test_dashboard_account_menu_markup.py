...
class DashboardAccountMenuMarkupTest(unittest.TestCase):
    ...
    def test_dashboard_task_list_filters_live_tasks_and_keeps_running_first(self):
        ...
        self.assertIn('id="showClosedTasks"', html)
        self.assertIn("Canlıya alınanları göster", html)
        self.assertIn("Geçmiş görevleri göster", html)
        ...
        self.assertIn("function taskListScope(list)", html)
        self.assertIn("if (!showLiveTasks.checked) scoped = scoped.filter(t => !taskIsLive(t));", html)
        self.assertIn("if (!showClosedTasks.checked) scoped = scoped.filter(t => !taskIsClosed(t));", html)
        self.assertIn("const allTasks = taskListScope(tasks());", html)
        self.assertIn("let list = taskListScope(tasks());", html)
        ...
        self.assertIn("[statusFilter, workerFilter, riskFilter, sortMode, showLiveTasks, showClosedTasks]", html)
        self.assertIn("if (el === showLiveTasks || el === showClosedTasks) fillFilters();", html)
