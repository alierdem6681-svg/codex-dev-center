...
class WorkerStatusModelTest(unittest.TestCase):
    ...
    def test_worker_claim_records_dispatch_ownership_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            queue_path = Path(tmp) / "task_queue.json"
            queue_path.write_text(
                json.dumps(
        ...
        self.assertEqual(claimed["id"], "TASK-CLAIM")
        self.assertEqual(payload["tasks"][0]["status"], TASK_STATUS_RUNNING)
        self.assertEqual(payload["tasks"][0]["worker_id"], "worker-1")
        self.assertEqual(payload["tasks"][0]["claimed_at"], payload["tasks"][0]["started_at"])
        self.assertTrue(payload["tasks"][0]["claimed_at"])
...
class BacklogDispatcherModelTest(unittest.TestCase):
    ...
    def test_existing_dispatch_child_prevents_duplicate_when_parent_pointer_is_missing(self):
        tasks = [
            {"id": "PARENT", "status": TASK_STATUS_READY_FOR_VALIDATION, "risk": "low"},
            {
                "id": "CHILD",
                "status": "PENDING",
    ...
    def test_existing_repo_apply_child_prevents_duplicate_when_parent_pointer_is_missing(self):
        tasks = [
            {"id": "PARENT", "status": TASK_STATUS_PROPOSAL_DONE, "risk": "low", "title": "safe app work"},
            {
                "id": "APPLY-CHILD",
                "status": "PENDING",
                "source": lifecycle_manager.BACKLOG_DISPATCHER_SOURCE,
    ...
    def test_proposal_done_prefers_repo_apply_child(self):
        ...
        self.assertEqual(child["root_task_id"], "PARENT")
        self.assertEqual(child["dispatch_id"], child["id"])
        self.assertEqual(child["worker_id"], child["assigned_worker"])
        self.assertEqual(child["attempt"], 1)
        self.assertEqual(child["max_attempts"], 2)
        self.assertEqual(child["last_error_code"], "")
        self.assertIsNone(child["claimed_at"])
        self.assertIsNone(child["finished_at"])
